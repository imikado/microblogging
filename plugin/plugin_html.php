...
class plugin_html{
	...
	* @param array $tOption tableau contenant autant d'options a ajouter array('style'=>'border:1px')  >> <img style="border:1px"...
	* @return string retourne le code html de l'image
	*/
	public function getImg($sSrc,$sAlt=null,$tOption=null){
		if($sAlt==null){ $sAlt=$sSrc; }
		return '<img src="'._root::getConfigVar('path.img').$sSrc.'" title="'.$sAlt.'" '.$this->getOptionFromTab($tOption).'/>';
	}
	/** 
	* retourne le code html d'une div
	* @access public
	* @param string $sContenu contenu de la div
	* @param array $tOption tableau contenant autant d'options a ajouter array('style'=>'border:1px')  >> <div style="border:1px"...
	...
	* @param array $tOption tableau contenant autant d'options a ajouter array('style'=>'border:1px')  >> <input style="border:1px"...
	...
	*/
	public function getInputRadio($sName,$sValue=null,$bChecked=false,$tOption=null){
		...
		return '<input type="radio" name="'.$sName.'" value="'.$sValue.'" '.$sChecked.' '.$this->getOptionFromTab($tOption).'/>';
	}
	/** 
	...
	*/
	public function getInputCheckbox($sName,$sValue=null,$bChecked=false,$tOption=null){
		...
		$sChecked='';
		if($bChecked) $sChecked='checked="checked"';
		return '<input type="checkbox" name="'.$sName.'" value="'.$sValue.'" '.$sChecked.' '.$this->getOptionFromTab($tOption).'/>';
	}
	/** 
	...
	public function getSelect($sName,$tSelect,$sValue=null,$tOption=null){
		...
		$sHtml='<select name="'.$sName.'"'.$this->getOptionFromTab($tOption).'>';
			...
			foreach($tSelect as $sKey => $sVal){
				$sHtml.='<option '; if($sKey==$sValue){ $sHtml.='selected="selected"';} $sHtml.=' value="'.$sKey.'">'.$sVal.'</option>';
			}
		$sHtml.='</select>';
		return $sHtml;
	}
	...
	*/
	public function getSelectMultiple($sName,$tSelect,$tValue=null,$tOption=null){
		...
		$sHtml='<select multiple="multiple" name="'.$sName.'[]"'.$this->getOptionFromTab($tOption).'>';
			if($tSelect)
			foreach($tSelect as $sKey => $sVal){
				$sHtml.='<option '; if(is_array($tValue) and in_array($sKey,$tValue)){ $sHtml.='selected="selected"';} $sHtml.=' value="'.$sKey.'">'.$sVal.'</option>';
			}
		$sHtml.='</select>';
		return $sHtml;
	}
	...
	* @param array $tOption tableau contenant autant d'options a ajouter array('style'=>'border:1px')  >> <textarea style="border:1px"...
	* @return string retourne le code html de l'input
	*/
	...
	*/
	public function getDateFromInput($tPost,$sName){
		...
		return $tPost[$sName.'_annee'].'-'.$tPost[$sName.'_mois'].'-'.$tPost[$sName.'_jour'];
	}
